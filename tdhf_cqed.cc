...
int read_options(std::string name, Options& options)
{
    if (name == "MEAN_FIELD_CQED"|| options.read_globals()) {
        ...
        /*- amplitude of second pulse in a.u.-*/
        options.add_double("LASER_AMP2", 0.0);
        /*- frequency of pulse in a.u. (default is the 1 fs pulse) -*/
        options.add_double("LASER_FREQ", 0.1519829846);
        /*- frequency of second pulse in a.u. (default is the 1 fs pulse) -*/
        options.add_double("LASER_FREQ2", 0.1519829846);
        /*- delay between two laser pulses -*/
        options.add_double("LASER_DELAY", 2067.07);
